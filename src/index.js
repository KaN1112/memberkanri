...
const { configureGuild, getConfiguredGuildIds, getGuildConfig, upsertMember } = require("./storage");
...
async function postRecord(guildId, embed) {
  const config = getGuildConfig(guildId);
  if (!config.recordChannelId) return false;

  const channel = await client.channels.fetch(config.recordChannelId);
...
async function registerXForMember(member, url, registeredBy) {
  ...
  await postRecord(member.guild.id, buildXRegisteredEmbed(saved, registeredBy));
  await updateMemberList(client, member.guild.id);
  return saved;
}

async function requireGuildSetup(interaction) {
  const config = getGuildConfig(interaction.guild.id);
  if (config.recordChannelId && config.membersChannelId) return true;

  await interaction.reply({
    content: "このサーバーはまだ初期設定されていません。管理者が `/setup-channels` を実行してください。",
    ephemeral: true
  });
  return false;
}
...
client.once(Events.ClientReady, async () => {
  ...
  for (const guildId of getConfiguredGuildIds()) {
    try {
      await updateMemberList(client, guildId);
    } catch (error) {
      console.warn(`サーバー ${guildId} のメンバー一覧を更新できませんでした。`, error.message);
    }
  }
});
...
client.on(Events.GuildMemberAdd, async (member) => {
  try {
    ...
    if (!config.recordChannelId || !config.membersChannelId) return;

    upsertMember(member);
    await postRecord(member.guild.id, buildJoinEmbed(member));
    await updateMemberList(client, member.guild.id);
  } catch (error) {
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (!interaction.guild) {
    await interaction.reply({ content: "このコマンドはサーバー内で実行してください。", ephemeral: true });
    return;
  }

  if (interaction.commandName === "setup-channels") {
    const recordChannel = interaction.options.getChannel("record_channel", true);
    const membersChannel = interaction.options.getChannel("members_channel", true);

    configureGuild(interaction.guild.id, {
      recordChannelId: recordChannel.id,
      membersChannelId: membersChannel.id,
      resetListMessage: true
    });

    await updateMemberList(client, interaction.guild.id);
    await interaction.reply({
      content: `このサーバーの設定を保存しました。\n記録チャンネル: ${recordChannel}\n一覧チャンネル: ${membersChannel}`,
      ephemeral: true
    });
  }

  if (interaction.commandName === "register-x") {
    if (!(await requireGuildSetup(interaction))) return;
  ...
  if (interaction.commandName === "members") {
    ...
    await interaction.reply({ embeds: [buildMemberListEmbed(interaction.guild.id)], ephemeral: true });
  }
